...
from sp500_forecastability import recovery_v3_6_2 as pace
...
def test_protocol_has_fixed_balanced_development_and_test_sizes() -> None:
    assert pace.EXPECTED_COUNTS == {"development": 600, "test": 400}
    assert pace.EXPECTED_LABELS["test"] == {"Supported": 200, "Refuted": 200}
...
def test_failed_certificate_is_fail_closed_for_features_and_routing() -> None:
    ...
    certificates[0] = {
        ...
        "success": False,
        "certificate": None,
        "attempts": [{"parse_error": "truncated"}, {"parse_error": "truncated"}],
        "final_error": "ValueError: truncated",
    }
    rows = pace._certificate_groups(certificates)
    assert not pace._certificate_gate(rows[("pace-x", "candidate_0")], "no")
    names, matrix, _keys = pace._feature_matrix(
        [_inference_example()], _baseline_records(), certificates
    )
    relation_index = names.index("certificate_relation_insufficient")
    assert matrix[0, relation_index] == 1.0
...
def test_ex_fever_selection_passes_all_structural_gates() -> None:
    ...
    assert report["test_distinct_page_roots"] == 800
    assert report["v3_6_1_claim_overlap"] == 0
    assert report["v3_6_1_page_root_overlap"] == 0
