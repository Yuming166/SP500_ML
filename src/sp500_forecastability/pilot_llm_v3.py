"""Forced-answer Pilot-LLM V3 paired-intervention runner."""
...
PROTOCOL_VERSION = "pilot-llm-v3-2026-08-31"
DEFAULT_ROOT = Path("results/pilot_llm_v3")
...
class ForcedQADecision:
    """Strict model output before any downstream selective routing."""
    ...
    def to_internal_payload(self) -> dict[str, object]:
        """Add evaluator-only decision metadata not present in the model schema."""
...
def build_manifest(dataset_path: Path) -> dict[str, object]:
    ...
    return {
        ...
        "output_contract": "forced_yes_no_before_router",
        "model": DEFAULT_MODEL,
...
def validate_manifest(
    ...
    for field, expected_value in expected_fields.items():
        if manifest.get(field) != expected_value:
            raise ValueError(f"manifest {field} does not match Pilot-LLM V3")
    ...
    if manifest.get("examples") != expected_rows:
        raise ValueError("manifest examples do not match the recomputed V3 sample")
    if Counter(example.label for example in expected) != Counter({False: 25, True: 25}):
        raise ValueError("V3 manifest must contain 25 examples per label")
    return expected
...
def build_messages(
    ...
    *,
    ...
    """Build a stateless question-only prompt that requires a yes/no answer."""
    ...
    system = (
        ...
        "When evidence is empty or insufficient, use your best general-knowledge judgment. "
        "You must choose yes or no; you cannot abstain. "
    ...
    user = (
        ...
        "answer must be exactly 'yes' or 'no'. Confidence is the probability that your answer "
        "is correct. cited_evidence_ids must be a unique list drawn only from the packet; use "
        "an empty list when relying only on general knowledge.\n\n"
        f"agent_id must be exactly: {agent_id}\n"
        "Required keys, with no others: agent_id, answer, confidence, cited_evidence_ids.\n\n"
...
def run_one_call(
    ...
    *,
    ...
    repair = False
    decision: ForcedQADecision | None = None
    ...
    return {
        ...
        "attempts": attempts,
        "decision": decision.to_internal_payload() if decision is not None else None,
    ...
    *,
    ...
    if mode == "smoke":
        ...
        if len(examples) != FORMAL_EXAMPLES:
            raise ValueError("formal mode requires the exact 50-question V3 manifest")
    ...
    report = render_report(summary).replace("# Pilot-LLM V1", "# Pilot-LLM V3", 1)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    prepare = subparsers.add_parser("prepare", help="write and validate the V3 manifest")
    ...
    smoke = subparsers.add_parser("smoke", help="run at most six V3 instrumentation calls")
    ...
    formal = subparsers.add_parser("run", help="run the frozen 750-call V3 pilot")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "prepare":
        ...
        print(f"Wrote frozen V3 manifest: {args.output}")
        return 0
