"""Pilot-LLM V6 preregistered paired-intervention runner.
...
endpoints (D_inert, D_conf, D_OR), and the secondary shared-citation detector
``shared_weighted`` (§9 of docs/pilot_llm_v6_preregistration.md).

V6 inherits the V5 protocol structure (partitions, condition set,
agent-persona set, FEVER dataset digest, instrumentation, scoring) and
replaces:

  - N = 50 -> 100 (balanced 50/50; D1_v6)
  - V5's co-primary (D_OR + shared_weighted) -> V6 single co-primary (D_OR
    only); shared_weighted demoted to secondary S4_v6 (D2_v6)
  - V5 §16 signpost (cross-model) -> V6 same-model N-scaled; cross-model
    is V7 (D3_v6)
  - Substitute-generation budget <=150 calls -> <=300 calls preregistered,
    ~13K realized due to per-sentence cardinality (D4_v6)
  - Salt pilot-llm-v5-2026-08-31 -> pilot-llm-v6-2026-09-01 (D5_v6);
    selection is re-seeded (V5 ⊂ V6 is NOT guaranteed by construction)

Substitute generation runs **before** the formal run as a separate phase
(<=300 LLM calls preregistered; ~13K realized; no formal-run budget impact).
See §6.3 / §6.4 of the V6 prereg.
"""
...
PROTOCOL_VERSION = "pilot-llm-v6-2026-09-01"
SALT = b"pilot-llm-v6-2026-09-01\n"
...
DEFAULT_ROOT = Path("results/pilot_llm_v6")
    ...
    for ent, d in by_cluster.items():
        for label, members in d.items():
            ...
            for i in range(0, len(members_sorted) - FACTS_PER_QUESTION + 1,
                ...
                cqid = f"v6-{maj[:3].lower()}-{cqid_seed}"
...
def build_manifest(
    ...
    return {
        ...
        "co_primary_endpoints": ["D_OR"],  # V6 D2_v6: single co-primary
        "secondary_endpoints": ["shared_weighted", "shared_citation_signal"],
        "examples": [
            {
                "cqid": comp.cqid,
                "label": comp.label,
...
def validate_manifest(
    ...
    if manifest.get("protocol_version") != PROTOCOL_VERSION:
        raise ValueError("manifest is not Pilot-LLM V6")
    ...
    if manifest.get("n_agents") != N_AGENTS:
        raise ValueError("manifest n_agents does not match V6 protocol")
    if manifest.get("facts_per_question") != FACTS_PER_QUESTION:
        raise ValueError("manifest facts_per_question does not match V6 protocol")
    if manifest.get("facts_per_agent") != FACTS_PER_AGENT:
        raise ValueError("manifest facts_per_agent does not match V6 protocol")
    if manifest.get("partition_table") != [sorted(s) for s in PARTITION_TABLE]:
        raise ValueError("manifest partition_table does not match V6 protocol")
    ...
    if counter["SUPPORTS"] != FORMAL_PER_LABEL or counter["REFUTES"] != FORMAL_PER_LABEL:
        raise ValueError(
            f"V6 manifest must contain {FORMAL_PER_LABEL} composites per label; "
            f"got {dict(counter)}"
        )
...
def _loao_aurocs(rows):
    """V6: LOAO is reported for D_OR only (single co-primary).

    shared_weighted LOAO is included as a secondary observation but not
    required by §9.4.
    """
    out = []
    if not rows:
...
def summarize_records(
    ...
    for field, target in [
        ...
        ("shared_weighted", "harmful_fc"),         # V6 S4 secondary
    ...
    # V6 §9.2 single co-primary verdict logic.
    # Only D_OR must clear; shared_weighted is reported as secondary S4_v6.
    ...
    if rows:
        ...
        co_primary_verdict = {
            ...
            "shared_weighted_secondary": {
                "auroc": metric_block[sw_key]["auroc"],
                "ci_lo": sw_ci[0], "ci_hi": sw_ci[1],
                "note": "V6 §9.3 S4: reported, not gating; CI lo > 0.5 is bonus",
            },
            "verdict": "PASS" if or_pass else "FAIL",
        }
...
def render_report(summary) -> str:
    lines = [f"# Pilot-LLM V6 {summary['mode']} report", ""]
    ...
    lines.append("")
    lines.append("## Co-primary verdict (V6 §9.2: D_OR single co-primary)")
    ...
    if cpv is None:
        ...
        sw = cpv.get('shared_weighted_secondary', {})
        lines.append(f"- shared_weighted (V6 §9.3 S4 secondary) AUROC = "
                     f"{sw.get('auroc', 'NA')} "
                     f"[{sw.get('ci_lo', 'NA'):.3f}, {sw.get('ci_hi', 'NA'):.3f}] "
                     f"— {sw.get('note', '')}")
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "These results test whether V5's methodology scales on the same model "
        "(Qwen3.5-4B) at N = 100. They do not establish LLM faithfulness in "
        "general, S&P 500 predictability, investment performance, or "
        "cross-model generalization. The `shared_weighted` secondary signal "
        "has a structural variance ceiling on FEVER (see V6 §13 and "
        "`results/pilot_llm_v5/scaling_check.json`); V6 reports it but does "
        "not gate §9.2 on it. Cross-model generalization is the V7 prereg."
    )
    return "\n".join(lines) + "\n"
...
def _build_parser():
    ...
    prep = sub.add_parser("prepare", help="write the frozen V6 manifest")
