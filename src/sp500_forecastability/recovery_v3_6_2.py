"""Fail-closed proof-carrying PACE on a fresh EX-FEVER test split."""
...
from sp500_forecastability import recovery_v3_6_1 as v361
from sp500_forecastability.pilot_llm_v1 import _attempt_payload, _extract_json_object
from sp500_forecastability.recovery_v2 import PERSONAS, RecoveryChatClient, parse_decision

PROTOCOL_VERSION = "recovery-v3.6.2-fail-closed-atomic-pace-ex-fever-2026-09-02"
DEFAULT_ROOT = Path("results/recovery_v3_6_2")
...
PREREGISTRATION = Path("docs/recovery_v3_6_2_preregistration.md")
PRIOR_SELECTION = Path("results/recovery_v3_6_1/selection_manifest.json")
PRIOR_ROUTER_MANIFEST = Path("results/recovery_v3_6_1/router/manifest.json")
DEVELOPMENT_RECORD_PROTOCOLS = frozenset({PROTOCOL_VERSION, v361.PROTOCOL_VERSION})
...
EXPECTED_COUNTS = {"development": 600, "test": 400}
EXPECTED_LABELS = {
    "development": {"Supported": 300, "Refuted": 300},
    "test": {"Supported": 200, "Refuted": 200},
}
SELECTION_SALT = b"pace-v3.6.2-fresh-test-selection-2026-09-02"
...
CERTIFICATE_FIELDS = {"atomic_checks", "coverage_complete", "confidence"}
ATOMIC_FIELDS = {"claim_span", "status", "evidence_ids"}
ATOM_STATUSES = ("supported", "contradicted", "unresolved")
RELATIONS = ("supports", "refutes", "insufficient", "conflicted")
MAX_DOC_SENTENCES = 3
MAX_SENTENCE_CHARS = 1_200
ATOMIC_MAX_COMPLETION_TOKENS = 512
FAIL_CLOSED_CERTIFICATE = {
...
def _select_fresh_component_test(
    ...
    *,
    ...
    for example in examples:
        component = union_find.find(next(iter(example["annotated_evidence"])))
        components[component].append(example)

    eligible: dict[str, list[Mapping[str, Any]]] = {}
    for component, rows in components.items():
        component_roots = {
            _normalise_root(root) for row in rows for root in row["annotated_evidence"]
        }
        if component_roots & excluded_roots:
            continue
        candidates = [
            row
            for row in rows
            if v34._normalise_claim(row["claim"]) not in excluded_claims
        ]
        if candidates:
            eligible[component] = candidates

    by_signature: dict[tuple[str, ...], list[str]] = defaultdict(list)
    for component, rows in eligible.items():
        signature = tuple(sorted({str(row["label"]) for row in rows}))
        by_signature[signature].append(component)
    for components_for_signature in by_signature.values():
        components_for_signature.sort(
            key=lambda component: base._hash_key(
                SELECTION_SALT, f"component\0{component}"
            )
        )

    assigned: dict[str, str] = {}
    for label in ("Supported", "Refuted"):
        for component in by_signature.get((label,), []):
            assigned[component] = label
    remaining = {
        label: EXPECTED_LABELS["test"][label]
        - sum(assigned_label == label for assigned_label in assigned.values())
        for label in ("Supported", "Refuted")
    }
    mixed = by_signature.get(("Refuted", "Supported"), [])
    cursor = 0
    for label in ("Supported", "Refuted"):
        needed = remaining[label]
        for component in mixed[cursor : cursor + needed]:
            assigned[component] = label
        cursor += needed
    if any(
        sum(assigned_label == label for assigned_label in assigned.values())
        ...
        raise ValueError("insufficient untouched components for balanced fresh test")

    selected = []
    for component, label in assigned.items():
        row = min(
            (row for row in eligible[component] if row["label"] == label),
            key=lambda row: base._hash_key(
                SELECTION_SALT, f"item\0{component}\0{row['example_id']}"
            ),
        )
...
def build_selection() -> dict[str, Any]:
    ...
    prior_selection = json.loads(PRIOR_SELECTION.read_text(encoding="utf-8"))
    v361.validate_selection(prior_selection)
    prior_examples = list(prior_selection["examples"])
    development = [dict(row) for row in prior_examples if row["split"] == "development"]
    exposed_claims = {v34._normalise_claim(row["claim"]) for row in prior_examples}
    exposed_roots = {
        _normalise_root(packet["root"])
        for row in prior_examples
    ...
    test_raw = _select_fresh_component_test(
        eligible,
        excluded_claims=exposed_claims,
        excluded_roots=climate_roots | exposed_roots,
    )
    examples = [
        *development,
    ...
    return {
        ...
        "status": "fresh_test_frozen_after_v3_6_1_schema_abort_before_new_test_calls",
        "datasets": {
        ...
        "split_rule": (
            "development is the frozen V3.6.1 development split; test is a deterministic "
            "balanced one-item-per-evidence-component sample excluding every V3.6.1 claim "
            "and component plus all CLIMATE-FEVER page roots"
        ),
        "prior_exposed_selection": {
            "path": str(PRIOR_SELECTION),
            "sha256": base._sha256_path(PRIOR_SELECTION),
        },
...
def validate_selection(selection: Mapping[str, Any]) -> None:
    if dict(selection) != build_selection():
        raise ValueError("Recovery V3.6.2 selection or source data drifted")
...
def audit_selection(selection: Mapping[str, Any]) -> dict[str, Any]:
    ...
    v361.validate_selection(prior_selection)
    exposed_claims = {
        v34._normalise_claim(row["claim"]) for row in prior_selection["examples"]
    }
    exposed_roots = {
        _normalise_root(packet["root"])
        for row in prior_selection["examples"]
        for packet in (row["anchor"], *row["candidates"])
    }
    new_test_claims = claims["test"]
    new_test_roots = {_normalise_root(root) for root in roots["test"]}
    ...
    gates = {
        ...
        "zero_v3_6_1_claim_overlap": not (new_test_claims & exposed_claims),
        "zero_v3_6_1_page_root_overlap": not (new_test_roots & exposed_roots),
    ...
    return {
        ...
        "v3_6_1_claim_overlap": len(new_test_claims & exposed_claims),
        "v3_6_1_page_root_overlap": len(new_test_roots & exposed_roots),
...
def build_certificate_messages(
    ...
    system = (
        "You generate an atomic, proof-carrying certificate for a conjunctive claim. Split the "
        "claim into 2-8 material details and check every detail against ANCHOR plus PROPOSED NEW "
        "ROOT. One contradicted detail makes the complete claim false. Pay special attention to "
        "names, country/nationality, dates/years, numbers, negation, occupation, organization, "
        "studio, and relation direction. Use only packet evidence. Return exactly one JSON "
        "object with atomic_checks (a list of objects having exactly claim_span, status, and "
        "evidence_ids), coverage_complete (boolean), and confidence (number in [0,1]). Each "
        "claim_span must be copied verbatim from the claim. Status is supported, contradicted, "
        "or unresolved. Evidence IDs may come from either packet. Set coverage_complete true "
        "only if every material claim detail was checked. No reasoning or extra keys."
    )
    user = (
        f"Claim: {example['claim']}\n\n"
        f"ANCHOR:\n{base._packet(example['anchor']['evidence'])}\n\n"
        f"PROPOSED NEW ROOT:\n{base._packet(candidate['evidence'])}\n\n"
        "Build the atomic comparison. Do not assume the previous consensus or use outside facts."
    )
    if repair:
        user += (
            "\nYour previous response was invalid. Return only the required JSON object with "
            "valid field types, verbatim claim spans, and packet-local evidence IDs."
        )
...
def parse_certificate(
    ...
    raw_checks = payload["atomic_checks"]
    if not isinstance(raw_checks, list) or not 1 <= len(raw_checks) <= 12:
        raise TypeError("atomic_checks must contain 1-12 objects")
    allowed = set(allowed_ids)
    allowed_new = set(allowed_new_ids)
    checks = []
    dropped_ids = []
    invalid_span_count = 0
    seen_spans = set()
    for raw_check in raw_checks:
        if not isinstance(raw_check, Mapping) or set(raw_check) != ATOMIC_FIELDS:
            raise ValueError("each atomic check must have exactly the frozen fields")
        span = " ".join(str(raw_check["claim_span"]).split())
        location = claim.casefold().find(span.casefold())
        if not span or location < 0 or span.casefold() in seen_spans:
            invalid_span_count += 1
            continue
        status = str(raw_check["status"]).casefold().replace("-", "_")
        status = {
            "support": "supported",
            "supports": "supported",
        ...
        ids = raw_check["evidence_ids"]
        if not isinstance(ids, list) or any(not isinstance(item, str) for item in ids):
            raise TypeError("atom evidence_ids must be a list of strings")
        local_ids = []
        ...
        if not local_ids:
            status = "unresolved"
        seen_spans.add(span.casefold())
        checks.append(
            {
                "claim_span": claim[location : location + len(span)],
                "status": status,
                "evidence_ids": local_ids,
            }
        )
    if not checks:
        raise ValueError("no valid atomic checks remain")
    counts = Counter(check["status"] for check in checks)
    contradicted = [check for check in checks if check["status"] == "contradicted"]
    supported = [check for check in checks if check["status"] == "supported"]
    coverage_complete = bool(payload["coverage_complete"]) and invalid_span_count == 0
    if contradicted:
        relation = "refutes"
        relation_checks = contradicted
    elif coverage_complete and len(checks) >= 2 and len(supported) == len(checks):
        relation = "supports"
        relation_checks = supported
    else:
        relation = "insufficient"
        relation_checks = []
    relation_new_ids = []
    for check in relation_checks:
        for evidence_id in check["evidence_ids"]:
            if evidence_id in allowed_new and evidence_id not in relation_new_ids:
                relation_new_ids.append(evidence_id)
    support_fraction = counts["supported"] / len(checks)
    return {
        "relation": relation,
        "support_strength": confidence * support_fraction,
        "refute_strength": confidence if contradicted else 0.0,
        "confidence": confidence,
        "new_evidence_ids": relation_new_ids,
        "dropped_evidence_ids": dropped_ids,
        "missing_bridge": relation == "insufficient",
        "atomic_checks": checks,
        "coverage_complete": coverage_complete,
        "atom_count": len(checks),
...
def _call_certificate_with_retry(
    ...
    candidate = example["candidates"][int(action[-1])]
    allowed_new_ids = [str(item["evidence_id"]) for item in candidate["evidence"]]
    allowed_ids = [
    ...
    for attempt_index in range(2):
        ...
        try:
            ...
            certificate = parse_certificate(
                result.content, str(example["claim"]), allowed_ids, allowed_new_ids
            )
...
def parse_action_decision(content: str, allowed_ids: Sequence[str]) -> dict[str, Any]:
    ...
    if not isinstance(citations, list) or any(
        ...
        return parse_decision(content, allowed_ids)
    allowed = set(allowed_ids)
    local_ids = []
    dropped_ids = []
    ...
    normalised["cited_evidence_ids"] = local_ids
    decision = parse_decision(json.dumps(normalised), allowed_ids)
    decision["dropped_evidence_ids"] = dropped_ids
    return decision


def _call_action_with_retry(
    client: RecoveryChatClient,
    ...
    *,
    ...
    for attempt_index in range(2):
        ...
        try:
            ...
            attempt = _attempt_payload(result)
            decision = parse_action_decision(result.content, allowed_ids)
        except (RuntimeError, TypeError, ValueError) as error:
            final_error = f"{type(error).__name__}: {error}"
            if attempt is None:
                attempt = {
...
def _run_action_example(
    ...
    for agent_index in range(len(PERSONAS)):
        decision, attempts, final_error = _call_action_with_retry(
            client,
...
def _validate_certificates(
    ...
    for example_id, rows in grouped.items():
        ...
        allowed_protocols = (
            DEVELOPMENT_RECORD_PROTOCOLS if split == "development" else {PROTOCOL_VERSION}
        )
        for row in rows:
            if row.get("protocol_version") not in allowed_protocols or row.get("split") != split:
                raise ValueError(f"{split} invalid certificate metadata for {example_id}")
            if row.get("success"):
                if row.get("certificate") is None:
                    raise ValueError(f"{split} successful certificate is empty for {example_id}")
                continue
            if (
                ...
                raise ValueError(f"{split} invalid fail-closed certificate for {example_id}")
...
def _validate_actions(
    ...
    for example_id, rows in grouped.items():
        ...
        if any(
            row.get("protocol_version") not in allowed_protocols
...
def _execute_bundles(
    ...
    *,
    ...
    certificate_run = run_one is _run_certificate_example

    def terminal(rows: Sequence[Mapping[str, Any]]) -> bool:
    ...
    existing = {
        example_id: rows for example_id, rows in by_example.items() if terminal(rows)
    }
    ...
    if run_one is _run_certificate_example:
        client.max_completion_tokens = ATOMIC_MAX_COMPLETION_TOKENS
    ...
    if len(records) != expected_rows or not terminal(records):
...
def execute(
    ...
    *,
    ...
    if not audit_selection(selection)["passed"]:
        raise ValueError("V3.6.2 selection failed structural gates")
...
def _feature_vector(
    ...
    certificate = certificate_row.get("certificate") or FAIL_CLOSED_CERTIFICATE
    ...
    names = [
        ...
        "certificate_supported_fraction",
        "certificate_contradicted_fraction",
        "certificate_unresolved_fraction",
        "certificate_coverage_complete",
        "certificate_invalid_span_count",
    ]
    values = np.asarray(
        [
            float(consensus == "yes"),
            ...
            counter - same,
            grounded * counter,
            float(certificate["atom_count"]),
            float(certificate["supported_count"]) / float(certificate["atom_count"]),
            float(certificate["contradicted_count"])
            / float(certificate["atom_count"]),
            float(certificate["unresolved_count"]) / float(certificate["atom_count"]),
            float(certificate["coverage_complete"]),
            float(certificate["invalid_span_count"]),
        ],
        dtype=float,
    )
    if any(
...
def _certificate_gate(certificate_row: Mapping[str, Any], consensus: str) -> bool:
    certificate = certificate_row.get("certificate")
    if not certificate_row.get("success", certificate is not None) or certificate is None:
        return False
...
def fit_router(
    ...
    if not PREREGISTRATION.exists():
        raise ValueError("V3.6.2 preregistration must exist before fitting")
    selection = json.loads(selection_path.read_text(encoding="utf-8"))
    if not audit_selection(selection)["passed"]:
        raise ValueError("V3.6.2 selection failed structural gates")
    ...
    names, _x, keys = _feature_matrix(examples, actions, certificates)
    prior_manifest = json.loads(PRIOR_ROUTER_MANIFEST.read_text(encoding="utf-8"))
    v361.validate_router_manifest(PRIOR_ROUTER_MANIFEST, PRIOR_SELECTION)
    if prior_manifest["development_action_records_sha256"] != base._sha256_path(
        action_records_path
    ) or prior_manifest["development_certificate_records_sha256"] != base._sha256_path(
        certificate_records_path
    ):
        raise ValueError("V3.6.2 development records differ from the frozen V3.6.1 router")
    if names != prior_manifest["feature_names"] or len(keys) != 1_200:
        raise ValueError("V3.6.2 feature schema differs from the frozen V3.6.1 router")
    model_path = Path(str(prior_manifest["router_joblib"]))
    output_dir.mkdir(parents=True, exist_ok=True)
    manifest = {
        "protocol_version": PROTOCOL_VERSION,
        "status": "frozen_before_fresh_ex_fever_test_calls",
        ...
        "training": {
            **prior_manifest["training"],
            "development_record_protocols": sorted(
                {str(row["protocol_version"]) for row in (*actions, *certificates)}
            ),
            "reused_without_refit_from": str(PRIOR_ROUTER_MANIFEST),
            "prior_router_manifest_sha256": base._sha256_path(PRIOR_ROUTER_MANIFEST),
        },
        "feature_names": names,
        "policy": prior_manifest["policy"],
        ...
        "feature_boundary": {
            ...
            "certificate_failure_is_deterministic_keep": True,
        },
        "claim_boundary": {
            "test_outcomes_seen": False,
            "fresh_test_certificates_generated": False,
            "v3_6_1_schema_abort_excluded_from_fresh_test": True,
...
def validate_router_manifest(manifest_path: Path, selection_path: Path) -> None:
    ...
    if manifest.get("status") != "frozen_before_fresh_ex_fever_test_calls":
        raise ValueError("PACE is not frozen before fresh formal test calls")
    ...
    if manifest.get("dependency_sha256") != expected_dependencies:
        raise ValueError("PACE runtime dependencies drifted")
...
def evaluate(
    ...
    preoutcome = {
        ...
        "fail_closed_certificate_rows": sum(
            not bool(row.get("success")) for row in certificates
        ),
        "policies": policies,
        "predictions": {
    ...
    summary = {
        "protocol_version": PROTOCOL_VERSION,
        "analysis_status": "prospective_fresh_root_disjoint_ex_fever_test",
        "selection_sha256": base._sha256_path(selection_path),
        "router_manifest_sha256": base._sha256_path(manifest_path),
        "test_action_records_sha256": base._sha256_path(action_records_path),
        "test_certificate_records_sha256": base._sha256_path(certificate_records_path),
        ...
        "certificate_validity": {
            ...
            "valid_rows": sum(bool(row.get("success")) for row in certificates),
            "fail_closed_rows": sum(not bool(row.get("success")) for row in certificates),
            "fail_closed_examples": len(
                {str(row["example_id"]) for row in certificates if not row.get("success")}
            ),
        },
        "policies": metrics,
        "primary_gates": gates,
        "passes": all(gates.values()),
        "verdict": "PASS_FAIL_CLOSED_ATOMIC_PACE_V3_6_2"
        if all(gates.values())
        else "NO_VERIFIED_ATOMIC_PACE_DOMINANCE",
