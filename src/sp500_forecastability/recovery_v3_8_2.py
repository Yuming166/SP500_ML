"""Transport-only V3.8.2 semantic JSON conformance for Qwen-to-Ling ELAR."""
...
PROTOCOL_VERSION = "recovery-v3.8.2-qwen-to-ling-elar-2026-09-02"
DEFAULT_ROOT = Path("results/recovery_v3_8_2_ling")
PREREGISTRATION = Path("docs/recovery_v3_8_2_preregistration.md")
RUN_SCRIPT = Path("scripts/run_recovery_v3_8_2.sh")
V381_MANIFEST = Path("results/recovery_v3_8_1_ling/protocol_manifest.json")
V381_ABORT = Path("results/recovery_v3_8_1_ling/ABORTED.md")
V381_PARTIAL = Path(
    "results/recovery_v3_8_1_ling/formal/actions/records.partial.jsonl"
)
V381_CACHE = Path("results/recovery_v3_8_1_ling/cache")
...
def parse_action_decision(content: str, allowed_ids: Sequence[str]) -> dict[str, Any]:
    """Apply the closed semantic JSON conformance layer, then parse strictly."""
    ...
    if modes:
        decision["parse_mode"] = "v3_8_2_" + "_and_".join(modes)
    return decision
...
def _audit_v381_abort() -> dict[str, Any]:
    """Bind the amendment to outcome-blind V3.8.1 transport evidence."""
    records = base._load_jsonl(V381_PARTIAL)
    ...
    expected = {
        "rows": 1040,
        "complete_example_bundles": 130,
        "terminal_failures": 1,
        "terminal_error_counts": {
            "TypeError: cited_evidence_ids must be a list of strings": 1
        },
    }
    if audit != expected:
        raise ValueError("V3.8.1 outcome-blind abort facts drifted")
    cache_artifacts = []
    for attempt in failures[0]["attempts"]:
        cache_key = str(attempt["cache_key"])
        cache_path = V381_CACHE / f"{cache_key}.json"
        response = json.loads(cache_path.read_text(encoding="utf-8"))
        payload = v362._extract_json_object(str(response["content"]))
        if payload.get("cited_evidence_ids") != "":
            raise ValueError("V3.8.1 failed response is not an empty citation string")
        cache_artifacts.append(
            {
                "cache_key": cache_key,
                "sha256": base._sha256_path(cache_path),
                "cited_evidence_ids_type": "string",
                "cited_evidence_ids_value": "",
            }
        )
    return {
        "partial_action_records": str(V381_PARTIAL),
        "partial_action_records_sha256": base._sha256_path(V381_PARTIAL),
...
def _build_protocol_manifest() -> dict[str, Any]:
    ...
    manifest["status"] = "frozen_before_any_v3_8_2_target_task_call"
    manifest["amendment"] = {
        "scope": "closed_semantic_json_conformance",
        "runner_script": str(RUN_SCRIPT),
        "runner_script_sha256": base._sha256_path(RUN_SCRIPT),
        "prior_protocol_manifest": str(V381_MANIFEST),
        "prior_protocol_manifest_sha256": base._sha256_path(V381_MANIFEST),
        "prior_abort_record": str(V381_ABORT),
        "prior_abort_record_sha256": base._sha256_path(V381_ABORT),
        "prior_abort_audit": _audit_v381_abort(),
...
def validate_protocol_manifest(
    ...
    with _configured_base():
        ...
        if actual != _build_protocol_manifest():
            raise ValueError("V3.8.2 manifest or a frozen dependency drifted")
...
def prepare(path: Path = DEFAULT_ROOT / "protocol_manifest.json") -> dict[str, Any]:
    with _configured_base():
        ...
        if not path.exists() and any(formal_root.glob("**/records*.jsonl")):
            raise ValueError("cannot freeze V3.8.2 after target formal records exist")
...
def evaluate() -> dict[str, Any]:
    with _configured_base():
        ...
        summary["verdict"] = (
            "PASS_ZERO_SHOT_CROSS_MODEL_ELAR_V3_8_2_SEMANTIC_CONFORMANCE"
            if summary["passes"]
            else "NO_VERIFIED_CROSS_MODEL_ELAR_TRANSFER_V3_8_2"
        )
        ...
        report = report_path.read_text(encoding="utf-8").replace(
            "# Recovery V3.8 result:", "# Recovery V3.8.2 result:", 1
        )
        ...
        report += "\n".join(
            [
                "## Closed semantic JSON conformance",
                "",
                (
                    "The fresh run used only the four preregistered, "
                    "semantic-preserving canonicalizations."
                ),
                f"Accepted action parse modes: `{json.dumps(parse_modes, sort_keys=True)}`.",
                (
                    "Both prior schema aborts are content-addressed in the manifest; "
                    "none of their target responses were reused at inference."
                ),
                "",
            ]
        )
