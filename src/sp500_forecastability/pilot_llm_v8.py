"""Pilot-LLM V7 preregistered paired-intervention runner.

FEVER (Wikipedia-claim fact verification) binary composites from the
``copenlu/fever_gold_evidence`` HF mirror, partitioned evidence packets
(5 agents x 2-of-3 subsets of FEVER evidence sentences), four conditions
(original / remove / reverse / substitute), three co-registered risk
endpoints (D_inert, D_conf, D_OR), and the co-primary shared-citation
detector ``shared_weighted`` (§9 of docs/pilot_llm_v7_preregistration.md).

V7 inherits the V5 protocol structure (partitions, condition set,
agent-persona set, FEVER dataset digest, instrumentation, scoring) and
V6's N=100 design, but with three critical differences from V6:

  - Selection salt is V5's (pilot-llm-v5-2026-08-31), so V5 ⊂ V7 by
    construction at the question level (D1_v7).
  - Co-primary endpoints restored to V5's D_OR + shared_weighted (D3_v7).
  - Verdict logic is V5 §9.2 any-passes (either co-primary clears = PASS),
    not V6's original single co-primary (which was amended post-run).

V7 isolates the selection variable (held fixed at V5's salt) from the N
variable (scaled 50 -> 100). This is the cleanest test of whether V5's
D_OR = 0.656 signal holds under V5's same questions at N = 100.
"""
...
from sp500_forecastability.pilot_llm_v1 import (
    DEFAULT_MODEL as V1_DEFAULT_MODEL,
    REPAIR_SUFFIX,
    EvidenceView,
    _agent_seed,
    _attempt_payload,
...
# --- V8 custom client (D5_v8, D2_v8) ------------------------------- #

class LingChatClient:
    """OpenAI-compatible chat client for Ling via SGLang.

    Mirrors sp500_forecastability.pilot_llm_v1.CachedChatClient but:
      - Bypasses V1's hardcoded endpoint/model validation (D5_v8)
      - Adds `chat_template_kwargs={"enable_thinking": False}` to every
        request (D2_v8) — Ling's chat_template emits <think>...</think>
        blocks by default; without disabling, the JSON contract breaks
      - Uses DEFAULT_ENDPOINT / DEFAULT_MODEL from this module (Ling)
        instead of V1's frozen values (Qwen3.5-4B)
    """

    def __init__(self, endpoint: str, model: str, cache_dir: Path,
                 timeout: float = 60.0,
        ...
        # Match V1's cache file naming: <cache_dir>/<sha256>.json
        self.cache_dir.mkdir(parents=True, exist_ok=True)

    def call(self, messages: Sequence[Mapping[str, str]], *, seed: int) -> "LingChatResult":
        import hashlib
        ...
        if self.chat_template_kwargs:
            request_payload["chat_template_kwargs"] = self.chat_template_kwargs
        cache_material = {"endpoint": self.endpoint, "request": request_payload}
        cache_key = hashlib.sha256(_canonical_json(cache_material).encode()).hexdigest()
        cache_path = self.cache_dir / f"{cache_key}.json"
        if cache_path.exists():
            cached = _json.loads(cache_path.read_text(encoding="utf-8"))
            return LingChatResult(
        ...
        req = _ur.Request(self.endpoint, data=body, headers=headers, method="POST")
        ...
        result = LingChatResult(
            content=content,
            model=payload.get("model", self.model),
            usage=usage,
            http_status=http_status,
            request_bytes=len(body),
            response_bytes=len(_json.dumps(payload).encode()),
...
# Backwards-compat alias used by the V1 interface (V8 is a fork of V1)
CachedChatClient = LingChatClient

# --- V6 constants ---------------------------------------------------------- #

PROTOCOL_VERSION = "pilot-llm-v8-2026-09-01"
SALT = b"pilot-llm-v5-2026-08-31\n"  # V8 D1_v8: V5's salt for V5 ⊂ V7 ⊂ V8
# CQID hash uses V5's PROTOCOL_VERSION (NOT V8's) so V5 ⊂ V7 ⊂ V8 by cqid sort
CQID_PROTOCOL_VERSION = "pilot-llm-v5-2026-08-31"
CQID_PREFIX = "v5"  # cqids are v5-prefixed for V5 ⊂ V7 ⊂ V8 compat
...
SUBSTITUTE_FAIL_FAST_PCT = 0.10      # V8 §6.3 (inherited from V5)

# V8 endpoint: SGLang server hosting Ling-3.0-tiny-int4 (D1_v8, D6_v8).
# Started externally (not by this module): see scripts/start_sglang_ling.sh.
DEFAULT_ENDPOINT = "http://localhost:31520/v1/chat/completions"
DEFAULT_MODEL = "Ling-3.0-tiny"  # served-model-name set by sglang launcher

# V8 §7 / D2_v8: Ling's chat_template emits <think>...</think> blocks by
# default; we must disable thinking to keep the JSON contract clean.
THINKING_CHAT_TEMPLATE_KWARGS = {"enable_thinking": False}

DEFAULT_DATASET = Path(
    "/storage/gaoym/sp500-forecastability-lab/data/fever/fever-validation.jsonl"
)
DEFAULT_ROOT = Path("results/pilot_llm_v8")
...
def load_fever(path: Path) -> list[FEVERItem]:
    ...
    NEI rows are excluded (V5 §4.1, inherited). Each row's first evidence
    sentence is taken as the per-row evidence item (V5 §4.2, inherited).
    """
...
def build_composite_questions(
    ...
    """V6 §4.3: per cluster, salt-sort, group every 3 rows as a composite.

    Returns the top-50 composites per label (SUPPORTS / REFUTES). Items
    without a successful substitute are excluded upstream.
    """
    eligible = [
        it for it in items
