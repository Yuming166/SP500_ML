"""Pilot-LLM V5 preregistered paired-intervention runner.

FEVER (Wikipedia-claim fact verification) binary composites from the
``copenlu/fever_gold_evidence`` HF mirror, partitioned evidence
packets (5 agents x 2-of-3 subsets of FEVER evidence sentences), four
conditions (original / remove / reverse / substitute), three co-registered
risk endpoints (D_inert, D_conf, D_OR), and the co-primary shared-citation
detector ``shared_weighted`` (§9 of docs/pilot_llm_v5_preregistration.md).

V5 inherits the V4 protocol structure (partitions, condition set,
agent-persona set, instrumentation) and replaces:

  - TruthfulQA loader -> FEVER loader
  - same-cluster opposite-label substitute -> LLM-rewritten negative
    paraphrase of the source evidence sentence (§6.3 of the V5 prereg)
  - V4's single primary (D_OR) -> V5's co-primary (D_OR + shared_weighted)
    with Bonferroni-style verdict logic in §9.2.

Substitute generation runs **before** the formal run as a separate
phase (≤150 LLM calls, no formal-run budget impact). See §6.3 / §6.4.
"""
...
PROTOCOL_VERSION = "pilot-llm-v5-2026-08-31"
SALT = b"pilot-llm-v5-2026-08-31\n"
BOOTSTRAP_SEED = 20_260_902          # V5 §11.0
...
SUBSTITUTE_FAIL_FAST_PCT = 0.10      # V5 §6.3

DEFAULT_DATASET = Path(
    "/storage/gaoym/sp500-forecastability-lab/data/fever/fever-validation.jsonl"
)
DEFAULT_ROOT = Path("results/pilot_llm_v5")

# Same partition table as V4: 5 agents x 2-of-3 subsets of {E01, E02, E03}.
...
class FEVERItem:
    qid: str                     # V5 internal qid (sha256 of claim[:64])
    claim: str
    evidence_sentence: str       # first annotated evidence sentence
    label: str                   # "SUPPORTS" or "REFUTES" (NEI excluded)
    entity: str                  # cluster key = evidence[0][0]


def load_fever(path: Path) -> list[FEVERItem]:
    """Load + binarize the FEVER gold-evidence validation set.

    NEI rows are excluded (V5 §4.1). Each row's first evidence sentence
    is taken as the per-row evidence item (V5 §4.2).
    """
    ...
    with path.open(encoding="utf-8") as handle:
        for line_no, line in enumerate(handle, start=1):
            ...
            label = row.get("label")
            if label not in ("SUPPORTS", "REFUTES"):  # NEI excluded
                continue
            claim = str(row.get("claim", "")).strip()
            ev = row.get("evidence") or []
            if not claim or not ev or not isinstance(ev[0], list) or len(ev[0]) < 3:
                raise ValueError(
                    f"line {line_no}: missing claim or evidence[0][2] sentence"
                )
            evidence_sentence = str(ev[0][2]).strip()
            entity = str(ev[0][0])
            qid = "fever-" + sha256(claim.encode()).hexdigest()[:16]
            if qid in seen_qids:
                # duplicate claim; skip silently (rare in FEVER)
                continue
            seen_qids.add(qid)
            items.append(FEVERItem(
                qid=qid, claim=claim, evidence_sentence=evidence_sentence,
...
class CompositeQuestion:
    cqid: str
    question_text: str        # concatenates the 3 claims + verify framing
    items: tuple[FEVERItem, ...]
    label: str                # "SUPPORTS" or "REFUTES"

    @property
    def gold_binary(self) -> int:
        # REFUTES=1 (positive for harmful_fc), SUPPORTS=0; matches V5 §4.1.
        return 1 if self.label == "REFUTES" else 0
...
def _composite_label_majority(items: Sequence[FEVERItem]) -> str | None:
    """V5 §4.3 / D2_v5: 2-1 splits are dropped (returns None). 3-0/0-3 kept."""
    counts = Counter(it.label for it in items)
    if counts["SUPPORTS"] == counts["REFUTES"]:
        return None
    return "SUPPORTS" if counts["SUPPORTS"] > counts["REFUTES"] else "REFUTES"


def build_composite_questions(
    items: Sequence[FEVERItem],
    substitute_manifest: Mapping[str, Mapping[str, str]],
) -> list[CompositeQuestion]:
    """V5 §4.3: per cluster, salt-sort, group every 3 rows as a composite.

    Returns the top-25 composites per label (SUPPORTS / REFUTES).
    Items without a successful substitute are excluded upstream.
    """
    eligible = [
        it for it in items
        if substitute_manifest.get(it.qid, {}).get("substitute_sentence")
    ]
    by_cluster: dict[str, dict[str, list[FEVERItem]]] = {}
    for it in eligible:
        by_cluster.setdefault(it.entity, {"SUPPORTS": [], "REFUTES": []})[it.label].append(it)
    ...
    for ent, d in by_cluster.items():
        for label, members in d.items():
            ...
            for i in range(0, len(members_sorted) - FACTS_PER_QUESTION + 1,
                ...
                if maj is None:  # 2-1 split (shouldn't happen here, but safety)
                    continue
                cqid_seed = sha256(
                    f"{PROTOCOL_VERSION}\n{ent}\n{i}".encode()
                ).hexdigest()[:10]
                cqid = f"v5-{maj[:3].lower()}-{cqid_seed}"
    ...
    if len([c for c in out if c.label == "SUPPORTS"]) < FORMAL_PER_LABEL:
        raise ValueError(
            f"insufficient SUPPORTS composites: have "
            f"{len([c for c in out if c.label == 'SUPPORTS'])}, "
            f"need {FORMAL_PER_LABEL}"
        )
    if len([c for c in out if c.label == "REFUTES"]) < FORMAL_PER_LABEL:
        raise ValueError(
            f"insufficient REFUTES composites: have "
            f"{len([c for c in out if c.label == 'REFUTES'])}, "
            f"need {FORMAL_PER_LABEL}"
        )
    return sorted(out, key=lambda c: c.cqid)


def _build_composite_question_text(triple: Sequence[FEVERItem]) -> str:
    """V5 §4.3: 'Verify the following three claims ... For each, respond yes
    if supported and no if refuted.' The agent must still emit a single
    decision over the composite (matching V4's single-decision contract)."""
    bullets = "\n".join(
        f"  ({i+1}) {it.claim}" for i, it in enumerate(triple)
    )
    return (
...
def _substitute_prompt(claim: str, evidence_sentence: str,
                       src_label: str) -> str:
    opp = "REFUTES" if src_label == "SUPPORTS" else "SUPPORTS"
    return (
        f"You will see a single sentence that was used as evidence for a "
        f"claim that the following statement is {src_label}: \"{claim}\".\n\n"
        f"Rewrite the evidence sentence so that it would support the "
        f"OPPOSITE verdict (i.e., evidence that the statement is {opp}), "
        "while keeping the sentence about the same topic and entity.\n\n"
        "Constraints:\n"
        ...
        f"Original evidence sentence:\n{evidence_sentence}"
    )


def _parse_substitute_response(content: str, src_tokens: int) -> str | None:
    """Extract the rewritten sentence; reject if length window violated."""
    text = content.strip()
    # Strip code fences if present.
    if text.startswith("```"):
        text = re.sub(r"^```[a-zA-Z]*\n?", "", text)
        text = re.sub(r"\n?```$", "", text)
    # Take the first non-empty paragraph as the rewrite.
    sentences = [s.strip() for s in re.split(r"[\n]+", text) if s.strip()]
    if not sentences:
        return None
    rewrite = sentences[0]
    # Remove a leading dash or bullet if the model added one.
    rewrite = re.sub(r"^[-*]\s+", "", rewrite)
    if not rewrite:
        return None
    # Length-window check (V5 §6.3 ±50%).
...
def build_substitute_manifest(
    ...
    *,
    ...
    Writes the substitute manifest **before any formal run** (V5 §6.3).
    Re-uses an existing manifest at ``cache_dir/substitute_manifest.json``
    if present, to keep the phase idempotent on resume.
    """
    ...
    for idx, item in enumerate(items):
        prompt = _substitute_prompt(item.claim, item.evidence_sentence, item.label)
        src_tokens = max(1, len(item.evidence_sentence.split()))
        try:
            result = client.call(
                [{"role": "user", "content": prompt}],
                seed=20_260_903,  # deterministic substitute generation
            )
            rewrite = _parse_substitute_response(result.content, src_tokens)
            # Accumulate transfer bytes if the client exposed them.
            if hasattr(result, "usage") and isinstance(result.usage, Mapping):
                pass  # chat client tracks bytes internally; not needed here.
        ...
        if rewrite is None:
            ...
            manifest[item.qid] = {
                ...
                "deviation_log": ["llm_negative_paraphrase_v5"],
...
def build_manifest(
    ...
    return {
        ...
        "co_primary_endpoints": ["D_OR", "shared_weighted"],   # V5 §9
        "examples": [
            {
                "cqid": comp.cqid,
                "label": comp.label,
...
def validate_manifest(
    ...
    if manifest.get("protocol_version") != PROTOCOL_VERSION:
        raise ValueError("manifest is not Pilot-LLM V5")
    ...
    if manifest.get("n_agents") != N_AGENTS:
        raise ValueError("manifest n_agents does not match V5 protocol")
    if manifest.get("facts_per_question") != FACTS_PER_QUESTION:
        raise ValueError("manifest facts_per_question does not match V5 protocol")
    if manifest.get("facts_per_agent") != FACTS_PER_AGENT:
        raise ValueError("manifest facts_per_agent does not match V5 protocol")
    if manifest.get("partition_table") != [sorted(s) for s in PARTITION_TABLE]:
        raise ValueError("manifest partition_table does not match V5 protocol")
    ...
    if counter["SUPPORTS"] != FORMAL_PER_LABEL or counter["REFUTES"] != FORMAL_PER_LABEL:
        raise ValueError(
            f"V5 manifest must contain {FORMAL_PER_LABEL} composites per label; "
            f"got {dict(counter)}"
        )
    ...
    for row in rows:
        items = tuple(
            FEVERItem(
                qid=item["qid"], claim=item["claim"],
                evidence_sentence=item["evidence_sentence"], label=item["label"],
                entity=item["entity"],
            )
            for item in row["items"]
        )
        # 2-1 tie audit (V5 §4.3 / D2_v5)
...
def build_evidence_view(
    ...
    for i, item in enumerate(composite.items):
        ...
        if condition == "original":
            text = item.evidence_sentence
        elif condition == "remove":
            continue
        elif condition == "reverse":
            text = f"Task-local counterfactual: it is false that: {item.evidence_sentence}"
        elif condition == "substitute":
            sub = substitute_manifest.get(item.qid, {}).get("substitute_sentence", "")
            text = sub if sub else item.evidence_sentence
        else:
...
def _per_question_risks(grouped):
    ...
    for cqid, recs in sorted(grouped.items()):
        ...
        # REFUTES=1 is positive for harmful_fc (V5 §4.1); correct iff cons matches.
        ...
        # V5 §9.1 shared_weighted = frac_shared*(1-correct) + 0.5*frac_shared*correct
        shared_weighted = frac_shared * (1 - correct) + 0.5 * frac_shared * correct
        shared_signal = frac_shared   # legacy V4-style, kept for traceability
...
def _question_bootstrap_ci(values, n_replicates=BOOTSTRAP_REPLICATES,
                           seed=BOOTSTRAP_SEED):
...
def _loao_aurocs(rows):
    ...
    for k in range(N_AGENTS):
        for endpoint in ("D_OR", "shared_weighted"):
            scores, labels = [], []
            for r in rows:
                ...
                else:  # shared_weighted LOAO is just the point estimate (no per-agent partition)
                    score = r["shared_weighted"]
                scores.append(score)
    ...
    return {
        ...
        "deviation_note": (
            ...
            "v5 single-call-per-agent design cannot re-assign subsets without "
...
def summarize_records(
    ...
    for field, target in [
        ...
        ("shared_weighted", "harmful_fc"),         # V5 co-primary
        ("shared_citation_signal", "harmful_fc"),  # legacy, kept for traceability
        ("D_OR", "any_wrong"),
    ...
    # V5 §9.2 co-primary verdict logic (Bonferroni-style: both must clear)
    ...
    if rows:
        ...
        co_primary_verdict = {
            ...
            "verdict": (
                "PASS_BOTH" if (or_pass and sw_pass)
                else "PARTIAL_PASS" if (or_pass or sw_pass)
                else "FAIL_BOTH"
            ),
        }
...
def render_report(summary) -> str:
    lines = [f"# Pilot-LLM V5 {summary['mode']} report", ""]
    ...
    lines.append("")
    lines.append("## Co-primary verdict (V5 §9.2: both endpoints must clear)")
    ...
    lines.append("")
    lines.append(
        "These results test whether V4's provenance-aware methodology "
        "generalizes across domain (TQA -> FEVER) on a single model "
        "(Qwen3.5-4B). They do not establish LLM faithfulness in general, "
...
def _pre_formal_audit(dataset_path, manifest_path, *,
    ...
    # Read substitute manifest from cache_dir (written by build_substitute_manifest).
...
def _build_parser():
    ...
    prep = sub.add_parser("prepare", help="write the frozen V5 manifest")
    ...
    subg = sub.add_parser(
        ...
        help="run the LLM-rewrite substitute-generation pass (V5 §6.3)",
    )
    ...
    smoke = sub.add_parser("smoke", help="run at most 8 V5 instrumentation calls")
    ...
    formal = sub.add_parser("run", help="run the frozen 1,000-call V5 formal pilot")
...
def main(argv=None):
    ...
    if args.command == "prepare":
        items = load_fever(args.dataset)
        # Load substitute manifest (may have been written by substitute-generation)
        ...
        print(f"Wrote frozen V5 manifest: {args.output}")
        return 0
    ...
    if args.command == "all":
        # 1. substitute-generation (idempotent if manifest exists at cache-dir)
        print("[all] step 1/5: substitute-generation (LLM rewrites)", flush=True)
        items = load_fever(args.dataset)
        ...
        # 2. prepare
        print("[all] step 2/5: prepare (frozen manifest)", flush=True)
        ...
        validate_manifest(manifest, args.dataset)
        print(f"[all] wrote frozen V5 manifest: {args.manifest}", flush=True)
        ...
        # 5. formal
        print("[all] step 5/5: formal (~1,000 calls + substitute-gen above)",
              flush=True)
