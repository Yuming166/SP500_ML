"""Proof-carrying PACE with atomic counterfactual repair certificates."""
...
PROTOCOL_VERSION = "recovery-v3.6.1-atomic-pace-ex-fever-development-2026-09-02"
DEFAULT_ROOT = Path("results/recovery_v3_6_1")
...
PREREGISTRATION = Path("docs/recovery_v3_6_1_preregistration.md")
DEPENDENCY_PATHS = tuple(
    Path(__file__).with_name(name)
    for name in ("pilot_llm_v1.py", "recovery_v2.py", "recovery_v3.py", "recovery_v3_4.py")
)
...
def validate_selection(selection: Mapping[str, Any]) -> None:
    if dict(selection) != build_selection():
        raise ValueError("Recovery V3.6 selection or source data drifted")
...
def execute(
    ...
    *,
    ...
    if not audit_selection(selection)["passed"]:
        raise ValueError("V3.6 selection failed structural gates")
...
def fit_router(
    ...
    if not PREREGISTRATION.exists():
        raise ValueError("V3.6 preregistration must exist before fitting")
    selection = json.loads(selection_path.read_text(encoding="utf-8"))
    if not audit_selection(selection)["passed"]:
        raise ValueError("V3.6 selection failed structural gates")
