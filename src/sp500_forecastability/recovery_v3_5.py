...
PROTOCOL_VERSION = "recovery-v3.5-pace-ex-fever-2026-09-02"
DEFAULT_ROOT = Path("results/recovery_v3_5")
...
PREREGISTRATION = Path("docs/recovery_v3_5_preregistration.md")
...
def parse_certificate(content: str, allowed_new_ids: Sequence[str]) -> dict[str, Any]:
    ...
    if len(ids) != len(set(ids)) or set(ids) - set(allowed_new_ids):
        raise ValueError("new_evidence_ids must be unique and local to the new root")
    if not isinstance(payload["missing_bridge"], bool):
        raise TypeError("missing_bridge must be boolean")
    return {
        "relation": relation,
        **numeric,
        "new_evidence_ids": ids,
