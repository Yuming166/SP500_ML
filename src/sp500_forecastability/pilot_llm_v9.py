...
PROTOCOL_VERSION = "pilot-llm-v9-2026-09-01"
SALT = b"pilot-llm-v9-2026-09-01\n"  # V9 D4_v9: fresh salt
# CQID hash uses V5's PROTOCOL_VERSION so V9 cqids DON'T collide with V5/V7
CQID_PROTOCOL_VERSION = "pilot-llm-v9-2026-09-01"
CQID_PREFIX = "v9"  # cqids are v9-prefixed (V9 selection is independent)

# V9 D1_v9: anti-herding cluster filter (locked at [0.10, 0.60])
# Computed from FEVER evidence_sentence strings ONLY — no LLM involvement.
# Locked at the exact band; no post-hoc relaxation is permitted.
ANTI_HERDING_MIN_OVERLAP = 0.10
ANTI_HERDING_MAX_OVERLAP = 0.60
...
DEFAULT_ROOT = Path("results/pilot_llm_v9")
...
def _evidence_overlap_max(triple: Sequence[FEVERItem]) -> float:
    """V9 §2 / D1_v9: max pairwise TF-IDF cosine similarity of the 3
    evidence sentences in a cluster. Pure structural metric — no LLM
    involvement. Implemented with stdlib + math.

    Returns max pairwise cosine ∈ [0, 1] (or 0.0 if any evidence is empty).
    """
    from collections import Counter

    texts = [it.evidence_sentence.lower() for it in triple]
    ...
    def tokenize(t: str) -> list[str]:
        ...
        toks = list(words)
        if len(words) >= 2:
            toks.extend([f"{words[i]}_{words[i+1]}" for i in range(len(words) - 1)])
        return toks

    tf = [Counter(tokenize(t)) for t in texts]
    df: Counter = Counter()
    for tfc in tf:
        for w in tfc:
            df[w] += 1
    n = len(texts)
    idf = {w: max(1.0, (n / max(1, c))) for w, c in df.items()}

    def vec(tfc: Counter) -> dict[str, float]:
        s = sum(tfc[w] * idf.get(w, 1.0) ** 2 for w in tfc)
        n2 = s ** 0.5
        if n2 == 0:
            return {}
        return {w: (tfc[w] * idf.get(w, 1.0)) / n2 for w in tfc}

    vecs = [vec(tfc) for tfc in tf]
    max_sim = 0.0
    for i in range(3):
        for j in range(i + 1, 3):
            ...
            sim = sum(v1[w] * v2[w] for w in common)
            if sim > max_sim:
                max_sim = sim
    return max_sim
...
def build_composite_questions(
...
) -> tuple[list[CompositeQuestion], dict[str, int]]:
    """V9 §4.3: per cluster, salt-sort, group every 3 rows as a composite,
    then apply §2 anti-herding cluster filter (cosine overlap ∈ [0.10, 0.60]).

    Returns the top-50 composites per label (SUPPORTS / REFUTES), filtered
    by the §2 anti-herding rule. Also returns a stats dict with
    `n_triples_total`, `n_triples_kept`, etc. for the audit (§11 of V9
    preregistration).
    """
    eligible = [
        it for it in items
    ...
    stats = {
        ...
        "n_triples_total": 0,
        "n_triples_kept": 0,
        "n_triples_dropped_high_overlap": 0,
        "n_triples_dropped_low_overlap": 0,
    }

    for ent, d in by_cluster.items():
        stats["n_clusters_total"] += 1
        for label, members in d.items():
            ...
            for i in range(0, len(members_sorted) - FACTS_PER_QUESTION + 1,
                ...
                if len(triple) != FACTS_PER_QUESTION:
                    continue
                stats["n_triples_total"] += 1

                # V9 §2: anti-herding cluster filter
                overlap = _evidence_overlap_max(triple)
                if overlap > ANTI_HERDING_MAX_OVERLAP:
                    stats["n_triples_dropped_high_overlap"] += 1
                    continue
                if overlap < ANTI_HERDING_MIN_OVERLAP:
                    stats["n_triples_dropped_low_overlap"] += 1
                    continue
                stats["n_triples_kept"] += 1
    ...
    n_sup = sum(1 for c in out if c.label == "SUPPORTS")
    n_ref = sum(1 for c in out if c.label == "REFUTES")
    if n_sup < FORMAL_PER_LABEL or n_ref < FORMAL_PER_LABEL:
        raise ValueError(
            f"V9 §11: anti-herding filter left insufficient balanced manifest: "
            f"SUPPORTS={n_sup}, REFUTES={n_ref}, target={FORMAL_PER_LABEL} per label. "
            f"Run aborted before any LLM call. Stats: {stats}"
        )
    return sorted(out, key=lambda c: c.cqid), stats
...
def _pre_formal_audit(dataset_path, manifest_path, *,
    ...
    composites, filter_stats = build_composite_questions(items, substitute_manifest)
