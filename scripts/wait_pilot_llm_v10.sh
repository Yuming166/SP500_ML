#!/usr/bin/env bash
# Block until the background Pilot-LLM V10 formal run finishes.
# Usage: bash scripts/wait_pilot_llm_v10.sh [--interval SECONDS]
#
# V10 special: prints the cross-model × cross-domain comparison
...
PID_FILE="results/pilot_llm_v10/formal/run.pid"
if [[ ! -f "${PID_FILE}" ]]; then
...
while kill -0 "${PID}" 2>/dev/null; do
    if [[ -f results/pilot_llm_v10/formal/progress.json ]]; then
        echo "---- $(date -Iseconds) ----"
        cat results/pilot_llm_v10/formal/progress.json
    fi
    sleep "${INTERVAL}"
done

echo "[wait] process ${PID} has exited"
if [[ -f results/pilot_llm_v10/formal/records.jsonl ]]; then
    echo "[wait] records.jsonl present; final summary at results/pilot_llm_v10/formal/summary.json"
    PY="${ROOT}/.venv/bin/python"
    [[ ! -x "${PY}" ]] && PY="$(command -v python3)"
    "${PY}" -c "
import json
s = json.load(open('results/pilot_llm_v10/formal/summary.json'))
print('=== V10 Co-primary verdict (V10 §9.2 any-passes; BoolQ on Qwen3.5-4B) ===')
...
print()
print('=== V10 §5 Cross-domain shared_weighted comparison ===')
...
if v10_sw:
    print(f'  V10 (BoolQ, N=100, V10 salt): {v10_sw.get(\"auroc\", 0):.3f} '
...
"
    ...
    echo "[wait] no records.jsonl; check results/pilot_llm_v10/formal/run.log"
    tail -50 results/pilot_llm_v10/formal/run.log
    exit 2
fi