#!/usr/bin/env bash
# V10.3 driver: inherited frozen selection, exact-source-length rewrite target,
# one fixed repair, then audit/smoke/formal only if all 300 rewrites pass.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "${ROOT}"
RESULTS="results/pilot_llm_v10_3"
mkdir -p "${RESULTS}"
LOG="${RESULTS}/all.log"
: > "${LOG}"
...
echo "[driver] V10.3 starts $(date -Iseconds); bg=${BG}; PY=${PY}" | tee -a "${LOG}"
echo "[driver] step 1/5: inherit and validate V10.1 selection" | tee -a "${LOG}"
"${PY}" -m sp500_forecastability.pilot_llm_v10_3 prepare 2>&1 | tee -a "${LOG}"
echo "[driver] step 2/5: exact-length rewrites + at most one fixed repair" | tee -a "${LOG}"
"${PY}" -m sp500_forecastability.pilot_llm_v10_3 substitute-generation 2>&1 | tee -a "${LOG}"
echo "[driver] step 3/5: pre-formal audit" | tee -a "${LOG}"
"${PY}" -m sp500_forecastability.pilot_llm_v10_3 audit 2>&1 | tee -a "${LOG}"
if [[ "${SKIP_SMOKE}" -eq 1 && -f "${RESULTS}/smoke/records.jsonl" ]]; then
    ...
    "${PY}" -m sp500_forecastability.pilot_llm_v10_3 smoke 2>&1 | tee -a "${LOG}"
fi
if [[ "${SKIP_FORMAL}" -eq 1 ]]; then
    echo "[driver] step 5/5: formal skipped" | tee -a "${LOG}"
    exit 0
fi
CMD=("${PY}" -m sp500_forecastability.pilot_llm_v10_3 run)
if [[ "${BG}" -eq 1 ]]; then
